...
def _ops_tested_by(test: ast.expr) -> set[str]:
    """The op literals a single `if`/`elif` test compares `op` against."""
    if not isinstance(test, ast.Compare) or len(test.ops) != 1:
        return set()
    if not (isinstance(test.left, ast.Name) and test.left.id == "op"):
        return set()
...
def analyze_store(envelopes, top_n: int = 20, samples_per_op: int = 4) -> dict:
    ...
    executed_effects = inert_effects = partial_effects = 0
    ...
    total_effects = executed_effects + partial_effects + inert_effects
    ranked = [
        {
            "op": op,
    ...
    partial_ranked = [
        {
            "op": op,
            "cards": partial_card_counts[op],
            "effects": partial_effect_counts[op],
            "in_vocabulary": op in known,
            "examples": samples.get(op, []),
        }
        for op, _ in sorted(partial_card_counts.items(), key=lambda kv: (-kv[1], kv[0]))
    ]
